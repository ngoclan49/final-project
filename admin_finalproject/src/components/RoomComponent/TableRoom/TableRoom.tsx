...
const TableRoom = ({ room }: Props) => {
  ...
  const renderLocation = useCallback((pos: number) => {
    const findLocation = locations.find(
      (position: LocationModel) => position.id === pos
    );
    return <>{findLocation?.tenViTri}</>;
  }, []);
  ...
  return (
    <>
      <tr>
        <td>
          {tenPhong.length > 20 ? tenPhong.substring(0, 20) + '...' : tenPhong}
        </td>
        <td>
        ...
        <td>{renderLocation(maViTri)}</td>
        <td>{khach}</td>
        <td>$ {giaTien}</td>
        <td>
          <div className='button'>
            <span onClick={handleDelete} className='buttonDel'>
              <BsTrash />
            </span>
            <span>
              <Link to={`/rooms/${id}`} className='buttonEdit'>
                <RiExchangeBoxLine />
              </Link>
            </span>
            <span onClick={handleOpen} className='buttonView'>
              <AiOutlineEye />
            </span>
          </div>
        </td>
